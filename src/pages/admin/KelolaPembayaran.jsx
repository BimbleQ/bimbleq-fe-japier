...
import { getJumlahTagihanPending, getPembayaran } from "../../services/AdminService";
...
const KelolaPembayaran = () => {
  ...
  const handleAction = (id, action) => {
    setPayments((prevPayments) => prevPayments.map((payment) => (payment.id === id ? { ...payment, status: action } : payment)));
  };

  const pendingPaymentsCount = payments.filter((payment) => payment.status === null).length;
  ...
  useEffect(() => {
    ...
    const fetchPembayaran = async () => {
      try {
        ...
        console.error("Terjadi kesalahan saat mengambil data jumlah kelas aktif:", error);
      }
    };
    ...
    <div className="p-6 bg-gray-100 h-full flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-lg shadow text-center">
          <h3 className="text-[#ff8c00] font-bold text-lg">Pembayaran Menunggu Verifikasi</h3>
          <p className="text-[#ff8c00] text-4xl font-bold">{jumlahTagihanPending}</p>
        </div>
      </div>

      {/* Tabel Daftar Pembayaran */}
      <div className="bg-white p-6 rounded-lg shadow">
        <h3 className="text-[#212121] font-bold text-lg mb-4">Daftar Pembayaran</h3>
        {error ? (
              ...
              <tbody>
                {getTagihan.map((item) => (
                  <tr key={item.id_pembayaran}>
                    <td className="py-2 px-4 border-b text-center">{item.id_pembayaran}</td>
                    <td className="py-2 px-4 border-b">{item.nama_siswa}</td>
                    <td className="py-2 px-4 border-b text-center">{item.tipe_kelas}</td>
                    <td className="py-2 px-4 border-b text-center">{item.tipe_pembayaran}</td>
                    ...
                    <td className={`py-2 px-4 border-b text-center ${item.status === "lunas" ? "text-green-500" : "text-red-500"}`}>{item.status}</td>
                    <td className="py-2 px-4 border-b text-center">
                      {item.link_bukti ? (
                        <a href={item.link_bukti} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">
                          Lihat Bukti
                        </a>
                      ) : (
                        "Tidak Ada"
